    ...
    <div style="display:flex; gap:1rem;">
      <form method="GET" style="display:flex; gap:0.5rem; flex-wrap:wrap;">
        ...
        <select name="sort" class="form-control" style="max-width:220px;">
          <?php $currentSort = $_GET['sort'] ?? 'recent'; ?>
          <option value="recent" <?= $currentSort === 'recent' ? 'selected' : '' ?>>Tri: plus recent</option>
          <option value="ancien" <?= $currentSort === 'ancien' ? 'selected' : '' ?>>Tri: plus ancien</option>
          <option value="budget_asc" <?= $currentSort === 'budget_asc' ? 'selected' : '' ?>>Prix: croissant</option>
          <option value="budget_desc" <?= $currentSort === 'budget_desc' ? 'selected' : '' ?>>Prix: decroissant</option>
        </select>
  ...
  <div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(320px, 1fr)); gap:1.5rem;">
    ...
    <div style="background:var(--bg-card); border:1px solid var(--border); border-radius:12px; padding:1.5rem; transition:all 0.3s; cursor:pointer; display:flex; flex-direction:column;"
      ...
      <div style="margin-top:1rem; display:grid; gap:0.5rem;">
        ...
        <a href="index.php?page=candidater&id=<?= (int)$offre['id_offre'] ?>" class="btn-outline" style="width:100%; text-align:center; text-decoration:none; padding:10px; border-radius:8px; display:inline-block; border:1px solid var(--accent-purple-light); color:var(--accent-purple-light);">
          Candidater
        </a>
        <?php endif; ?>
      </div>
