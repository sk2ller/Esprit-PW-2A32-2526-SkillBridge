...
$pendingCount = $sStats['en_attente'] ?? 0;
...
<aside class="admin-sidebar">
  ...
  <div class="sidebar-role-switcher">
    <a href="index.php?role=client&page=services" class="role-item <?= ($_SESSION['role'] ?? 'client') === 'client' ? 'active' : '' ?>" title="Mode Client">
      <i class="fas fa-user"></i>
      <span>Client</span>
    </a>
    <a href="index.php?role=freelancer&page=my_services" class="role-item <?= ($_SESSION['role'] ?? '') === 'freelancer' ? 'active' : '' ?>" title="Mode Freelancer">
  ...
  <nav class="sidebar-nav">
    ...
    <a href="index.php?page=admin_services" class="sidebar-item <?= $currentPage === 'admin_services' ? 'active' : '' ?>">
      <i class="fas fa-briefcase icon"></i> Services
      <?php if ($pendingCount > 0): ?>
      <span class="badge badge-warn"><?= $pendingCount ?></span>
      <?php endif; ?>
    </a>

    <a href="index.php?page=admin_offres" class="sidebar-item <?= strpos($currentPage, 'admin_offre') === 0 ? 'active' : '' ?>">
      <i class="fas fa-briefcase icon" style="color: #f97316;"></i> Offres Job
    </a>

    <a href="index.php?page=admin_categories" class="sidebar-item <?= $currentPage === 'admin_categories' ? 'active' : '' ?>">
      <i class="fas fa-tags icon"></i> Catégories
    </a>

    <div class="sidebar-section-label" style="margin-top:0.5rem;">Liens Rapides</div>

    <a href="index.php?page=services" class="sidebar-item" target="_blank">
