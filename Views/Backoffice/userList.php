...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        if (!isValidUserListName($nom) || !isValidUserListName($prenom)) {
            echo json_encode(['success' => false, 'message' => 'Le nom et le pr�nom ne doivent pas contenir de chiffres. Utilisez uniquement des lettres, espaces, apostrophes ou tirets']);
            exit;
        }
...
function showMappedFieldErrors(form, message) {
    ...
    if (message.indexOf('Le nom et le pr�nom doivent contenir') !== -1) {
        if (nomInput) appendFieldError(nomInput, 'Le nom ne doit pas contenir de chiffres');
        if (prenomInput) appendFieldError(prenomInput, 'Le pr�nom ne doit pas contenir de chiffres');
        return true;
    }
