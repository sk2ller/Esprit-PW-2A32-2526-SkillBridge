...
<div class="categories-bar">
  <a href="index.php?page=services" class="cat-chip <?= empty($_GET['categorie']) ? 'active' : '' ?>">
    <i class="fas fa-border-all"></i> Tous <span class="count"><?= count($services) ?></span>
  </a>
  <?php foreach ($categories as $cat): ?>
  <a href="index.php?page=services&categorie=<?= $cat['id_categorie'] ?>" class="cat-chip <?= (($_GET['categorie'] ?? '') == $cat['id_categorie']) ? 'active' : '' ?>">
...
<div class="page-container">
  ...
  <div class="section-header">
    ...
    <a href="index.php?page=services" style="color:var(--text-muted); font-size:0.875rem; text-decoration:none;">
      <i class="fas fa-times"></i> Effacer filtres
    </a>
  </div>
  <?php else: ?>
  <div class="section-header">
    <h2 class="section-title">Services <span>Populaires</span></h2>
  ...
  <div class="empty-state">
    ...
    <a href="index.php?page=services" class="btn-outline" style="display:inline-block; margin-top:1rem;">Voir tous les services</a>
  </div>
  <?php else: ?>
  <div class="services-grid">
    <?php foreach ($services as $s): ?>
    <div class="service-card">
      <div class="service-card-image" style="overflow:hidden; background: linear-gradient(135deg, <?= ['#1a0533','#0a2240','#002a1f','#1a1000'][crc32($s['titre']) % 4] ?>, var(--bg-secondary));">
