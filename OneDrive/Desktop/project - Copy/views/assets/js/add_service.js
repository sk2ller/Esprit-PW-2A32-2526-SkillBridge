// Validation for Adding Service Form
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  form.addEventListener("submit", function (e) {
    if (cvInput && cvInput.files.length > 0) {
      ...
      const ext = getExtension(cvInput.files[0].name);

      if (!allowedCv.includes(ext)) {
        alert("Le CV doit être en PDF, DOC ou DOCX.");
        e.preventDefault();
        return;
      }
    }
    ...
    if (portfolioInput && portfolioInput.files.length > 0) {
      const allowedPortfolio = ["pdf", "zip", "jpg", "jpeg", "png"];
      const ext = getExtension(portfolioInput.files[0].name);

      if (!allowedPortfolio.includes(ext)) {
        alert("Le portfolio doit être en PDF, ZIP, JPG, JPEG ou PNG.");
        e.preventDefault();
      }
    }
  });
...
if (thumbnailInput && thumbnailInput.files.length > 0) {
  ...
  if (!allowedThumb.includes(ext)) {
    alert("La miniature doit être en JPG, JPEG, PNG ou WEBP.");
    e.preventDefault();
    return;
  }
}
